...
bool ServerPlugin::Load(CreateInterfaceFn interfaceFactory, CreateInterfaceFn gameServerFactory)
{
    ...
    if (instances == 1)
    {
        ...
        /*success &=
            ...
            getInterface<IPlayerInfoManager>(gameServerFactory,interfaces.playerinfomanager,"PlayerInfoManager",2) &&
            // INTERFACEVERSION_VENGINESERVER
            getInterface<IVEngineServer>(interfaceFactory,interfaces.engine,"VEngineServer",21) &&
            // INTERFACEVERSION_GAMEEVENTSMANAGER2
            getInterface<IGameEventManager2>(interfaceFactory,interfaces.gameeventmanager2,"GAMEEVENTSMANAGER",2) &&
        ...
        success &=
            getInterface<IPlayerInfoManager>(gameServerFactory, interfaces.playerinfomanager,
                                             INTERFACEVERSION_PLAYERINFOMANAGER) &&
            getInterface<IVEngineServer>(interfaceFactory, interfaces.engine,
                                         INTERFACEVERSION_VENGINESERVER) &&
            getInterface<IGameEventManager2>(interfaceFactory, interfaces.gameeventmanager2,
                                             INTERFACEVERSION_GAMEEVENTSMANAGER2) &&
            getInterface<IServerPluginHelpers>(interfaceFactory, interfaces.helpers,
                                               INTERFACEVERSION_ISERVERPLUGINHELPERS) &&
            getInterface<IServerGameDLL>(gameServerFactory, interfaces.serverGameDll,
                                         INTERFACEVERSION_SERVERGAMEDLL) &&
            getInterface<IEngineSound>(interfaceFactory, interfaces.sounds,
                                       IENGINESOUND_SERVER_INTERFACE_VERSION) &&
            getInterface<IServerTools>(gameServerFactory, interfaces.serverTools,
                                       VSERVERTOOLS_INTERFACE_VERSION) &&
            getInterface<IFileSystem>(interfaceFactory, interfaces.filesystem,
                                      FILESYSTEM_INTERFACE_VERSION);
        ...
        if (success)
        {
            ...
            adminMenu = new Menu(NULL, "menu_administration",
                                 new MenuCallback<ServerPlugin>(this,
                                                                &ServerPlugin::adminMenuCallback));
            ...
            addPluginConVar(new I18nConVar(i18n, "cssmatch_version", CSSMATCH_VERSION,
                                           FCVAR_NOTIFY|FCVAR_REPLICATED, "cssmatch_version",
                                           cssmatch_version));
            addPluginConVar(new I18nConVar(i18n, "cssmatch_advanced", "1", FCVAR_NONE,
                                           "cssmatch_advanced", true, 0.0f, true, 1.0f));
            //addPluginConVar(new
            // I18nConVar(i18n,"cssmatch_gather","0",FCVAR_NONE,"cssmatch_gather",true,0.0f,true,1.0f));

            addPluginConVar(new I18nConVar(i18n, "cssmatch_report", "1", FCVAR_NONE,
                                           "cssmatch_report", true, 0.0f, true, 1.0f));

            addPluginConVar(new I18nConVar(i18n, "cssmatch_kniferound", "1", FCVAR_NONE,
                                           "cssmatch_kniferound", true, 0.0f, true, 1.0f));
            addPluginConVar(new I18nConVar(i18n, "cssmatch_kniferound_money", "0", FCVAR_NONE,
                                           "cssmatch_kniferound_money", true, 0.0f, true, 16000.0f));
            addPluginConVar(new I18nConVar(i18n, "cssmatch_kniferound_allows_c4", "1", FCVAR_NONE,
                                           "cssmatch_kniferound_allows_c4", true, 0.0f, true, 1.0f));
            addPluginConVar(new I18nConVar(i18n, "cssmatch_end_kniferound", "20", FCVAR_NONE,
                                           "cssmatch_end_kniferound", true, 0.0f, false, 0.0f));

            addPluginConVar(new I18nConVar(i18n, "cssmatch_rounds", "12", FCVAR_NONE,
                                           "cssmatch_rounds", true, 0.0f, false, 0.0f));
            addPluginConVar(new I18nConVar(i18n, "cssmatch_sets", "2", FCVAR_NONE, "cssmatch_sets",
                                           true, 0.0f, false, 0.0f));
            addPluginConVar(new I18nConVar(i18n, "cssmatch_end_set", "10", FCVAR_NONE,
                                           "cssmatch_end_set", true, 0.0f, false, 0.0f));

            addPluginConVar(new I18nConVar(i18n, "cssmatch_sourcetv", "1", FCVAR_NONE,
                                           "cssmatch_sourcetv", true, 0.0f, true, 1.0f));
            addPluginConVar(new I18nConVar(i18n, "cssmatch_sourcetv_path", "cfg/cssmatch/sourcetv",
                                           FCVAR_NONE, "cssmatch_sourcetv_path"));

            addPluginConVar(new I18nConVar(i18n, "cssmatch_warmup_time", "5", FCVAR_NONE,
                                           "cssmatch_warmup_time", true, 0.0f, false, 0.0f));

            addPluginConVar(new I18nConVar(i18n, "cssmatch_hostname", "", FCVAR_NONE,
                                           "cssmatch_hostname")); // Deprecated, use hostname instead
            addPluginConVar(new I18nConVar(i18n, "cssmatch_password", "", FCVAR_NONE,
                                           "cssmatch_password"));// Deprecated, use sv_password instead
            addPluginConVar(new I18nConVar(i18n, "cssmatch_default_config", "server.cfg",
                                           FCVAR_NONE, "cssmatch_default_config"));

            addPluginConVar(new I18nConVar(i18n, "cssmatch_usermessages", "28", FCVAR_NONE,
                                           "cssmatch_usermessages"));
            ...
            addPluginConCommand(new I18nConCommand(i18n, "cssm_restartmanche", cssm_restartmanche,
                                                   "cssm_restartmanche")); // backward  compatibility
            addPluginConCommand(new I18nConCommand(i18n, "cssm_restarthalf", cssm_restartmanche,
                                                   "cssm_restartmanche")); // same than cssm_restartmanche
            addPluginConCommand(new I18nConCommand(i18n, "cssm_restartround", cssm_restartround,
                                                   "cssm_restartround"));
            addPluginConCommand(new I18nConCommand(i18n, "cssm_adminlist", cssm_adminlist,
                                                   "cssm_adminlist"));
#ifdef _DEBUG
...
void ServerPlugin::showChangelevelMenu(Player * player)
{
    Menu * maplist = new Menu(adminMenu, "menu_map",
                              new MenuCallback<ServerPlugin>(this,
                                                             &ServerPlugin::changelevelMenuCallback));
...
void ServerPlugin::bantimeMenuCallback(Player * player, int choice, MenuLine * selected)
{
    if (choice != 10)
    {
        ...
        PlayerMenuLineData * const targetData =
            static_cast<PlayerMenuLineData * const>(player->getMenuData());
        ...
        CSSMATCH_VALID_PLAYER(PlayerHavingUserid, targetData->userid, target)
        {
            ...
            if (adminInfo != NULL)
            {
                ...
                if (time == 0)
                    i18n->i18nChatSay(recipients, "admin_permanently_ban", parameters,
                                      pIdentity->index);
                else
                    i18n->i18nChatSay(recipients, "admin_temporaly_ban", parameters,
                                      pIdentity->index);
            }
        }
        else
...
void ServerPlugin::addPluginClientCommand(const std::string & commandName,
...
void ServerPlugin::ClientPutInServer(edict_t * pEntity, char const * playername)
{
    ...
    if (isValidPlayerInfoIndex(index, interfaces.gpGlobals->maxClients))
    {
        ...
        try
        {
            ...
            bool isReferee = find(
                adminlist.begin(), invalidSteamid,
                interfaces.engine->GetPlayerNetworkIDString(pEntity)) != invalidSteamid;
...
void ServerPlugin::OnQueryCvarValueFinished(QueryCvarCookie_t iCookie, edict_t * pPlayerEntity,
