...
void Configuration::execute() const throw (CSSMatchConfigurationException)
{
	...
	if (Api::filesystem->FileExists(path.c_str(),"MOD"))
	{
		...
		Api::debug("Execution du fichier de configuration \"" + nomFichier + "\"...");
		Api::configExecute("cssmatch/configurations/" + nomFichier);
	}
	else
		// Si non (suppression pendant la manipulation du menu ?) on jette une exception
		throw CSSMatchConfigurationException("Impossible de trouver le fichier de configuration " + path);
}
