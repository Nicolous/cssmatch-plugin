	...
	if (test.empty())
	{
		...
		if (steamID != NULL)
		{
			...
			if (cssmatch->getAdminList()->estAdmin(steamID))
				...
				Messages::sayColor("Tu n'es pas arbitre du match !");
		}
	}
}
		...
		if (cssmatch->getAdminList()->estAdmin(steamID))
		{
			...
			if (match->getPhase() != PHASE_OFF)
			{
				...
				if (! nouvTag.empty())
				{
					...
					Messages::sayMsg("!teamt Le Nouveau Tag");
			}
			else
		...
		if (cssmatch->getAdminList()->estAdmin(steamID))
		{
			if (match->getPhase() != PHASE_OFF)
			{
				...
				if (! nouvTag.empty())
				{
					...
					Messages::sayMsg("!teamct Le Nouveau Tag");
			}
			else
		...
		if (cssmatch->getAdminList()->estAdmin(steamID))
		{
			...
			if (match->getPhase() != PHASE_OFF)
			{
				...
				if (! nouvTag.empty())
				{
					...
					Messages::sayTell(Api::engine->IndexOfEdict(pEntity),"Veuillez spécifier le nouveau tag de la team");
			}
			else
				Messages::sayMsg("Aucun match n'est en cours !");
		}
